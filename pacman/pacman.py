...
def readCommand(argv):
    ...
    #parser.add_option('--saveFile', dest='saveFile', type='int',
    #                  help=default('History Length)'), default=1)
    parser.add_option('--numHistory', dest='numHistory', type='int',
                      help=default('History Length)'), default=1)
    ...
    if options.numHistory > 0:
        args['numHistory'] = options.numHistory
        if 'numHistory' not in agentOpts:
            agentOpts['numHistory'] = options.numHistory
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, numHistory=1, catchExceptions=False, timeout=30):
    import __main__
