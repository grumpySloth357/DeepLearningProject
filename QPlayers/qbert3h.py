...
The network is composed of 3 hidden layers.
...
class DQNet():
    ...
    def build_nn(self):
        ...
        self.W3 = tf.Variable(tf.truncated_normal([256, 128], stddev = 0.1))
        self.b3 = tf.Variable(tf.truncated_normal([1, 128], stddev = 0.1))
        self.hidden3 = tf.nn.relu(tf.matmul(tf.reshape(self.hidden2,[-1, 256]), self.W3) + self.b3)
        self.W4 = tf.Variable(tf.truncated_normal([128, ACTION_SPACE], stddev = 0.1))
        self.b4 = tf.Variable(tf.truncated_normal([1, ACTION_SPACE], stddev = 0.1))
        self.output = tf.matmul(self.hidden3, self.W4) + self.b4
